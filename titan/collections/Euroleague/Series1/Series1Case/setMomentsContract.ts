...
const setMomentsContract = async (case_contract: ethers.Contract, moments_contract_address: string): Promise<boolean> => {
    Ducky.Debug(__filename, "setMomentsContract", `Setting case contract ${await case_contract.getAddress()} moments to ${moments_contract_address}.`)
    try {
        const tx: ethers.TransactionResponse = await case_contract.setMomentsContract(moments_contract_address)
        const receipt = await tx.wait()
        if (receipt !== null) {
            Ducky.Debug(__filename, "setMomentsContract", `Result: ${receipt.status === 1 ? "Success" : "Failure"}`)
        } else {
            Ducky.Error(__filename, "setMomentsContract", `Result: ${tx}`)
            throw new Error(`Failed to set moments contract ${moments_contract_address} to moments contract ${moments_contract_address}.`)
        }
        return true
    } catch (error: any) {
        Ducky.Error(__filename, "setMomentsContract", `Error: ${error}`)
        return false
    }
}
